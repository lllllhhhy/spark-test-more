...
object HSPersistToDisk {
  def main(args: Array[String]) {
    ...
    val PersistToDiskDir = args(2)

    //val size = sizeToSizeStr(outputSizeInBytes)


    val conf = new SparkConf()
      .setAppName(s"HSPersistToDisk")
      .registerKryoClasses(Array(classOf[Text])).setAppName("HSPersistToDisk")
      .setMaster("local") //新加的
      .set("spark.default.parallelism",parallelism)
      .set("spark.local.dir",PersistToDiskDir)
