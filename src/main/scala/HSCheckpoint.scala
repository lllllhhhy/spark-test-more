...
object HSCheckpoint {
  def main(args: Array[String]) {
    //System.setProperty("hadoop.home.dir", "C:/SoftWare/hadoopForWindows")
    if (args.length < 2) {
      println("Usage:")
    ...
    val parallelism=args(1).toInt match {
      case -1=>"50"
      case t=>String.valueOf(t)
    }
    val checkpointDir = "file:///"+args(2)
    print(checkpointDir)
    //val size = sizeToSizeStr(outputSizeInBytes)


    val conf = new SparkConf()
      .setAppName(s"HSCheckpoint")
      .registerKryoClasses(Array(classOf[Text])).setAppName("HSCheckpoint")
      .setMaster("local") //新加的
    ...
    try {
      ...
      if (!(recordsPerPartition < Int.MaxValue)) {
        throwsException(" HSCheckpoint Exception, records per partition > {Int.MaxValue}")
      }
      ...
      dataset.cache()
      dataset.checkpoint()
      val dataNew=dataset.map { (ele) =>
        ele.productPrefix
      }
      dataNew.count()

      //dataset.saveAsNewAPIHadoopFile[HSOutputFormat](outputFile)//调用hadoopAPI
    } catch{
      case e: Exception => println("Spark HSCheckpoint Exception" + e.getMessage() + e.printStackTrace())
    } finally {
      sc.stop()
    }
  ...
  /**
    ...
    * @return
    */
    def sizeStrToNumber(str: String): Long = {//将字符串解析成数字
  ...
  /**
    ...
    * @return
    */
  def sizeToSizeStr(size: Long): String = {//将传入的参数转为不同的单位
